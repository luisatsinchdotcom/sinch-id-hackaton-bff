...
app.get("/samples", (req, res) => {
  ...
  console.log('Returning samples for subject:', subject);
  console.log(recordedSamples[subject]);

  res.send(recordedSamples[subject] || []);
});
...
app.post("/challenges", jsonParser, async function (req, res) {
  if (req.body.input){
    ...
    if (req.body.subject) {
      ...
      // Valid Samples aren't shaped correctly yet
      // if (validSamples[subject]) {
      //   combinedSamples.push(...validSamples[subject]);
      // }
